...
let MailService = class MailService {
    ...
    resolveLogoPath() {
        ...
        if (process.env.MAIL_LOGO_PATH) {
            candidates.push((0, path_1.isAbsolute)(process.env.MAIL_LOGO_PATH)
                ? process.env.MAIL_LOGO_PATH
                : (0, path_1.join)(process.cwd(), process.env.MAIL_LOGO_PATH));
        }
        candidates.push((0, path_1.join)(process.cwd(), '..', 'frontend', 'src', 'assets', 'Wankil_Studio_Logo.png'), (0, path_1.join)(process.cwd(), '..', 'frontend', 'public', 'pwa-192.png'), (0, path_1.join)(process.cwd(), '..', 'frontend', 'public', 'favicon.png'), (0, path_1.join)(process.cwd(), 'public', 'favicon.png'));
    ...
    getLogoAttachments() {
        ...
        return [
            {
                filename: (0, path_1.basename)(logoPath),
                path: logoPath,
                cid: MAIL_LOGO_CID,
            },
        ];
  ...
  <body style="margin:0;padding:0;background:#070a12;color:#f4f7ff;font-family:Arial,Helvetica,sans-serif;">
    ...
    async sendVerificationCode(email, username, code) {
        const mail = this.buildAuthMail({
            ...
            intro: 'Entre ce code dans la page de vérification pour activer ton compte et récupérer ton bonus de départ.',
            codeLabel: 'Code de vérification',
            securityNote: "Si tu n'es pas à l'origine de cette création de compte, ignore simplement cet email.",
    ...
    async sendPasswordResetCode(email, username, code) {
        const mail = this.buildAuthMail({
            ...
            intro: 'Utilise ce code pour confirmer la réinitialisation de ton mot de passe Wankul TCG.',
            codeLabel: 'Code de réinitialisation',
            securityNote: "Si tu n'es pas à l'origine de cette demande, ton compte reste protégé et tu peux ignorer cet email.",
            ctaLabel: 'Retourner sur Wankul TCG',
        });
        await this.transporter.sendMail({
            from: this.getFrom(),
